...
- Use dendrogram of entire motif structure (as either a transparent underlay or a dnesity cloud), and highlight the edges and nodes which match a given search (*Challenge: time to render whole shebang*)
...
ntwk %>%
  activate(nodes) %>% 
  filter(row %in% trace_to_root("Seal")) %>%
  ...
  geom_node_text(
    aes(
      label = ifelse(
        level >= max(level) - 1, 
        str_trunc(as.character(description),30,"right"), 
        NA_character_
      )
    ),
    repel = T,size = 2,alpha = 0.5
  ) +
  coord_fixed() +
  theme_void()
